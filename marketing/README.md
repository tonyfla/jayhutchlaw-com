...
| `ads/` | Social media ads — one folder per ad. See `ads/README.md` |
...
`ads/` holds the ad files, one folder per ad. The only command:

```bash
python3 tools/adlink.py personal-injury /practice-areas/personal-injury/
```

It prints the UTM-tagged link to paste into the ad. See `ads/README.md`.
